  ...
  io.on('connection',
    function (socket) {
      ...
      socket.on('room:history',
        function (room) {
          return Models.Message.scope({method: ['lastDay']})
            .findAll({order: ['Message.createdAt'], where: {roomId: room}, include: [{model: Models.User, as: 'user'}]})
            .then(
              function (messages) {
      ...
      socket.on('message:send',
        function (object) {
          ...
          return Models.Message.create({roomId: room, userId: user.id, message: message})
            .then(
              function (message) {
                return io.sockets.in(room)
                  .emit('message:sent', room, message, user);
              }
            )
        }
      )
